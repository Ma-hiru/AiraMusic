...
必选参数 : id : 音乐 id level: 播放音质等级, 分为 standard => 标准,higher => 较高, exhigh=>极高, lossless=>无损, hires=>Hi-Res, jyeffect => 高清环绕声, sky => 沉浸环绕声, dolby => 杜比全景声, jymaster => 超清母带 unblock: 是否使用UnblockNeteaseMusic, 分为true和false
...
直接获取灰色歌曲链接
说明 : 技术来自于 UnblockNeteaseMusic 的支持, 聚合多个音源匹配(有几率匹配不成功)

必选参数 : id : 音乐 id

可选参数 : source: 选择要解灰的音源, 目前支持pyncmd, bodian, kuwo, kugou, qq, migu, 选择多个音源中间用,分隔
