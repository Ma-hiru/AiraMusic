...
export class MainWindow {
  ...
  static create() {
    const exits = WindowManager.checkAndShow("main");
    if (exits) return exits;

    Log.debug("Create APP Window");
    const params = Store.get("window");
    const { x, y } = params || {};
    const mainWindow = this.createWindow(params);
    ...
    return this.registerWindowEvents(mainWindow);
  }

  private static createWindow(params: { width?: number; height?: number }) {
  ...
  private static loadSource(mainWindow: BrowserWindow, port: number | string) {
  ...
  private static registerWindowEvents(mainWindow: BrowserWindow) {
