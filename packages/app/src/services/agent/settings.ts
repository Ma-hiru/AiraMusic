...
export class MainAgentFeatureSettings {
   ...
   * Agent 需要内部 MCP，因此 Agent 或公开 MCP 任一启用时都要监听回环端点。
   */
  static beginMcpInitialization(): undefined | AgentFeatureSettingsConfig {
    this.ensureCaptured();
    if (
      this.mcpInitializationAttempted ||
      (!this.startup?.agentEnabled && !this.startup?.mcpEnabled)
    ) {
      return undefined;
    }
...
function listAvailableMcpTools(): AgentFeatureSettingsMcpTool[] {
  const tools =
    availableMcpToolsCache ??
    (() => {
      // MCP 白名单可含破坏性工具，设置页需按完整目录展示可选项。
      const toolByName = new Map(
        createAgentToolCatalog(true).list.map((tool) => [tool.name, tool])
      );
      const available = AiraPublicMcpToolNames.flatMap((name) => {
        ...
        return [
          {
            ...
            description,
            risk: isAiraMcpDestructiveToolName(name)
              ? ("destructive" as const)
              : isAiraMcpMutatingToolName(name)
                ? ("write" as const)
                : ("read" as const)
          }
        ];
      });
      availableMcpToolsCache = available;
      return available;
    })();
